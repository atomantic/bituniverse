...
function DeepWidget({ title, subtitle, visibleLabel, visibleCount, totalLabel, totalCount, keysPerUnit, hoveredChild, hoveredLabel }) {
  return (
    <HudWidget>
      ...
      <Typography
        sx={{ color: "var(--theme-accent)", fontSize: "0.55rem", mb: 0.5, opacity: 0.7 }}
      >
        {subtitle}
      </Typography>
...
export function GlobeWidget({ continentId, hoveredChild }) {
  return (
    <DeepWidget
      title={`Zone #${parseInt(continentId).toLocaleString()}`}
      subtitle="Planetary Globe"
      visibleLabel="Visible Zones"
      visibleCount={VISIBLE_CONTINENTS}
      totalLabel="Total Continents"
      totalCount={CONTINENTS_PER_PLANET}
      keysPerUnit={KEYS_PER_CONTINENT}
      hoveredChild={hoveredChild}
      hoveredLabel="Continent"
    />
  );
}

export function ContinentWidget({ regionId, hoveredChild }) {
  return (
    <DeepWidget
      title={`Region #${parseInt(regionId).toLocaleString()}`}
      subtitle="Continental Map"
      visibleLabel="Visible Regions"
      visibleCount={VISIBLE_REGIONS}
      totalLabel="Total Regions"
      totalCount={REGIONS_PER_CONTINENT}
      keysPerUnit={KEYS_PER_REGION}
      hoveredChild={hoveredChild}
      hoveredLabel="Region"
    />
  );
...
export function GrainWidget({ moleculeId, hoveredChild }) {
  return (
    <DeepWidget
      ...
      subtitle="Crystal Lattice Interior"
...
export function QuarkWidget({ stringId, hexKey }) {
  return (
    <HudWidget>
      ...
      {hexKey && (
        <Box sx={{ mt: 0.5, pt: 0.5, borderTop: "1px solid rgba(77, 244, 255, 0.1)" }}>
          <Typography sx={{ color: "var(--theme-secondary)", fontSize: "0.55rem", mb: 0.25 }}>
            SHA-256 Key
          </Typography>
          <Typography
            sx={{
