...
function GluonStrings({ seed, onStringHover }) {
  ...
  const { positions, colors } = useMemo(() => {
    ...
    for (let i = 0; i < STRING_COUNT; i++) {
      const h1 = hashString(`${seedVal}s${i}x`);
      const h2 = hashString(`${seedVal}s${i}y`);
      const h3 = hashString(`${seedVal}s${i}z`);
      ...
      const hue = (i / STRING_COUNT) * 0.3 + 0.5;
...
function FluxTubes({ seed }) {
  ...
  const lineGeometry = useMemo(() => {
    ...
    for (let i = 0; i < STRING_COUNT; i += 3) {
      if (i + 1 < STRING_COUNT) {
  ...
  return (
    <lineSegments geometry={lineGeometry}>
      <lineBasicMaterial color="#ff61d8" opacity={0.06} transparent />
    </lineSegments>
  );
}
...
function StringTooltipHtml({ stringIndex, ...params }) {
  ...
  const { galaxyId, starId, planetId, continentId, regionId, areaId, groundId, grainId, moleculeId, atomId, quarkId } = params;
  const hexKey = computeHexKey(galaxyId, starId, planetId, continentId, regionId, areaId, groundId, grainId, moleculeId, atomId, quarkId, stringIndex);
  ...
  return (
    <Html center style={{ pointerEvents: "none" }} position={[0, -10, 0]}>
      ...
      >
        <div style={{ color: "#ff61d8", fontSize: "0.7rem", marginBottom: 4 }}>
          SHA-256 Key #{stringIndex.toLocaleString()}
        </div>
        <div
          style={{
            color: "#4df4ff",
        ...
        <div style={{ color: "#a999b3", fontSize: "0.55rem", marginTop: 4 }}>
          This is a unique Bitcoin private key.
        </div>
      </div>
    </Html>
  );
...
export default function QuarkView({ onStringHover }) {
  ...
  const seed = `${galaxyId}${starId}${planetId}c${continentId}r${regionId}a${areaId}g${groundId}gr${grainId}m${moleculeId}a${atomId}q${quarkId}`;

  const handleStringHover = (idx) => {
    setHoveredString(idx);
    onStringHover?.(idx);
  };
